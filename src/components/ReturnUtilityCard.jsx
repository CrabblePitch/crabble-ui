...
const ReturnUtilityCard = ({ rental }) => {
    if (!rental) return ;
    ...
    return (
        <Paper sx={{
            ...
            <UtilityCard
                data={rental}
                detailed={false}
                cardStyles={{ boxShadow: '', pointerEvents: 'none' }}
            />
            ...
            <ReturnUtilityButton styles={{ mt: 1, visibility: rental.phase === RentalPhase.LIQUIDATED ? 'hidden' : '' }}
                                 rental={rental}/>
        </Paper>
    )
};

export default ReturnUtilityCard;